locals {
  image_id       = length(var.aws_data.nat_instance_types["arm64"]) > 0 ? var.aws_data.amis["arm64"].id : var.aws_data.amis["amd64"].id
  instance_types = length(var.aws_data.nat_instance_types["arm64"]) > 0 ? var.aws_data.nat_instance_types["arm64"] : var.aws_data.nat_instance_types["amd64"]
  user_data      = <<EOF
#!/usr/bin/env bash
